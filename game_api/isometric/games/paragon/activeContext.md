...
- [x] **First playable data slice landed (2026-09-07)** — `games/paragon/` now has a real, validated game
  package (`game.json`…`assets.json`), authored against `QUEST_TREE.md` + `docs/content/` (Anchor Town
  Integrity token chain, Shrine of Compassion + healer companion), using the engine's generic
  `CoreStats`/`reward.stat_delta` mechanism for virtue counters — **no new engine code needed**.
  `python tools/validate_data.py games/paragon` → 0 errors/0 warnings. Synced into
  `game_api/isometric/games/paragon/`. **Not yet run**: no Godot binary in this environment — smoke test
  (`game_api/isometric/tools/godot.ps1 smoke paragon`) and `tools/run_tests.ps1` still need a real run
  before trusting the wiring; do that next on a machine with Godot 4.7+ installed
  (`game_api/isometric/godot.local.example.json`).
...
- [ ] **P0-T1 Project bootstrap** (H, S)
  - [ ] Lay out the game package under `games/paragon/` per `docs/ARCHITECTURE.md §3` (monorepo mapping);
        no separate `project.godot` — runs via `tools/sync_game.ps1 -Game paragon -Engine isometric` then
        `godot --path game_api/isometric -- --game=paragon`
  - [ ] Git LFS for `assets/**` (monorepo `.gitignore` already covers Godot)
  - [ ] Autoload stubs: `Game`, `EventBus`, `Settings`, `Audio` (check what `game_api/isometric/autoloads/`
        and `core/` already provide before adding)
  - [ ] One passing GUT test in `tests/` (repo standard; run with `tools/run_tests.ps1`)
  - [ ] `tools/validate_data.py` skeleton (walks `data/`, validates JSON parse + required `id`)
...
0. **Verify the slice on a Godot-equipped machine**: `godot.ps1 check`, `godot.ps1 smoke paragon`,
   `tools/run_tests.ps1` (all 3 GUT suites green). Fix any interaction-kind/behavior gaps the smoke run
   surfaces (this slice hasn't been engine-verified yet, only data-validated).
...
- 2026-09-06 — **Writer track begun (P3-W1..W5 done).** `docs/content/WRITERS_GUIDE.md`; full modules
  `virtues/integrity.md` (9 scenarios), `compassion.md` (8), `humility.md` (9) with companions, arcs, remarks,
  farewells, testimony, action tables, visions, quiz, twists, synonyms; `dilemmas/integrity_vs_kindness.md`
  (*The Dying Father*); `tokens/anchor_town_chain.md` (roles, keyword ladder, 2 true sources each, false
  variant + refutation, honesty gate, general pattern); `quests/heart_jailed_mother.md`,
  `quests/street_bar_fight.md` in full. Writer Track section added to `DEVELOPMENT_TASKS.md` (P3-W1..W11).
- 2026-09-06 — Win-state & conduct design: `CRITICAL_PATH.md`, `WITNESS_LOG.md`, `SIDE_QUESTS.md`; rules §3.22–26.
- 2026-09-06 — Virtue pool redesign: `VIRTUES.md` (15 → 8 per seed; Humility capstone; Wisdom meta-virtue; dilemmas).
