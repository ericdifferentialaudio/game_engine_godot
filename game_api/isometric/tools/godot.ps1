...
param(
    ...
    [ValidateSet('import', 'check', 'edit', 'run', 'smoke', 'version', 'raw', 'which')]
...
switch ($Command) {
    ...
    'smoke' {
        # Headless boot of a game package: generates the world, runs a few turns,
        # exercises intel/save/load and quits with a non-zero code on failure.
        $game = if ($Rest.Count -gt 0) { $Rest[0] } else { 'example_realm_iso' }
        exit (Invoke-Godot @('--headless', '--path', $Root, '--', "--game=$game", '--smoke'))
    }
